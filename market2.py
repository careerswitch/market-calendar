...
def update_calendar():
    try:
        ...
        if economics_calendar:
            save_calendar_to_gcs(economics_calendar)  # Added GCS upload
        else:
...
# Function to save the calendar to GCS
def save_calendar_to_gcs(calendar):
    try:
        # Save the calendar to a temporary local file
        filename = "Market_temp.ics"
        with open(filename, 'wb') as f:
            f.write(calendar.to_ical())

        # Upload the calendar file to GCS
        credentials_path = os.path.join(os.path.dirname(__file__), "GCP_SA_KEY")
        client = storage.Client.from_service_account_json(credentials_path)
        bucket_name = "market-calendar-bucket"
        bucket = client.get_bucket(bucket_name)
        blob = bucket.blob("Market.ics")
        blob.upload_from_filename(filename)

        logging.info(f"Calendar uploaded to Google Cloud Storage: gs://{bucket_name}/Market.ics")

        # Remove the local temporary file
        os.remove(filename)

    except Exception as e:
        logging.exception(f"An error occurred while saving the calendar to Google Cloud Storage: {str(e)}")
        raise
